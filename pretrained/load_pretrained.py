...
def load_small_model(device):
    ...
    model.load_state_dict(torch.load("./pretrained/model_small.pt", map_location=device))
    return model


def load_large_model(device):
    model = BasicTransformer(3015, 50, 5, 500, 4, 0.0).to(device=device)
    model.load_state_dict(torch.load("./pretrained/model_large.pt", map_location=device))
    return model